...
class TDMPC2(struct.PyTreeNode):
    ...
    def plan(
        ...
        actions = jnp.zeros((self.horizon, self.population_size, self.model.action_dim))
    ...
    def update(
        ...
        pixel_observations: jax.Array,
        actions: jax.Array,
        rewards: jax.Array,
        next_observations: jax.Array,
        next_pixel_observations: jax.Array,
        terminated: jax.Array,
        truncated: jax.Array,
        *,
        key: PRNGKeyArray,
    ) -> Tuple[TDMPC2, Dict[str, Any]]:
        world_model_key, policy_key, encoder_key = jax.random.split(key, 3)
        ...
        def world_model_loss_fn(
            ...
            z = self.model.encode(
                ...
                jax.tree.map(lambda x: x[0], pixel_observations),
                encoder_params,
                key=encoder_key,
            )
            zs = zs.at[0].set(z)
            ...
            for t in range(self.horizon):
                ...
                consistency_loss += self.rho**t * jnp.mean(
                    (z - next_z[t]) ** 2, where=~finished[t][:, None]
                )
