...
class SequentialReplayBuffer:
    def __init__(
        ...
        self.data = jax.tree.map(
            lambda x: np.zeros((capacity,) + np.asarray(x).shape, np.asarray(x).dtype),
            dummy_input,
        )
    ...
    def sample(self, batch_size: int, sequence_length: int) -> Dict:
        ...
        if self.size < self.capacity:
            ...
            buffer_starts = self.np_random.randint(0, self.size, size=(batch_size, 1))
            sequence_inds = buffer_starts + np.arange(sequence_length)
            sequence_inds = sequence_inds % self.capacity

        # Sample from buffer and convert from (batch, time, *) to (time, batch, *)
        batch = jax.tree.map(
            lambda x: np.swapaxes(x[sequence_inds, env_inds], 0, 1), self.data
        )
